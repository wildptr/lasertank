        ...
        case ID_LOADLEV_03: // Search
          ...
          if (i== 1) // idOk
          {
            ...
            while (BytesMoved == sizeof(TLEVEL))
            {
              ...
              if (strstr(_strupr(temps),SearchRec.data) && (TempRecData.SDiff & SearchRec.Diff )
...
LRESULT CALLBACK HSBox(HWND Dialog, UINT Message, WPARAM wparam, LPARAM lparam)
{
  ...
  switch(Message)
  {
    case WM_INITDIALOG:
      ...
      {
        strcpy(old,txt009);
        _itoa(GHS.moves,temps,10); strcat(old,temps); strcat(old,txt010);
        _itoa(GHS.shots,temps,10); strcat(old,temps); strcat(old,txt011);
        strcat(old,GHS.name);
...
LRESULT CALLBACK PBWindow(HWND Dialog, UINT Message, WPARAM wparam, LPARAM lparam)
{
  ...
  switch (Message)
  {
    case WM_INITDIALOG:
      ...
      {
		  _itoa(PBRec.Size,temps,10);
        SetWindowText(GetDlgItem(Dialog,ID_PLAYBOX_10),temps);
...
LRESULT CALLBACK PickBox(HWND Dialog, UINT Message, WPARAM wparam, LPARAM lparam)
{
  ...
  switch (Message)
  {
    case WM_INITDIALOG:
      ...
      if (CurLevel < i) _itoa(CurLevel,temps,10);
