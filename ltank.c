...
LRESULT CALLBACK WndProc(HWND Window, UINT Message, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_PAINT:
		...
		if ((CurLevel == 0) || QHELP )
		{
			...
			{
				_itoa(i,temps,10);
				TextOut(pdc,8,YOffset+y+((i-1) * SpBm_Height),temps,strlen(temps));
				if ( i<10 )
				{
					_itoa(i,temps,10);
					TextOut(pdc,8+XOffset+(16*SpBm_Width) ,YOffset+y+((i-1) * SpBm_Height),temps,strlen(temps));
				}
				else
				{
					strcpy(temps, "1 ");
	...
	case WM_MOVE:
		...
		_itoa(twp.rcNormalPosition.top,temps,10);
		WritePrivateProfileString("SCREEN",psYpos,temps,INIFile);
		_itoa(twp.rcNormalPosition.left,temps,10);
	...
	int       nCmdShow)
{
	...
	if (lpCmdLine[0] != 0)
	{
		...
		if (strstr(_strlwr(lpCmdLine),".lvl"))
		{
			AssignHSFile();
			CurLevel = 0;
			LoadNextLevel(FALSE,FALSE);
